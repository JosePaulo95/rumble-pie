...
const initItemsAvoidingRep = (prevArray, objetivoId) => {
  const bagitems = [1, 2, 3, 4].filter((item) => item !== objetivoId);
  let currentArray = Array.from(
    { length: 8 },
    () => bagitems[Math.floor(Math.random() * bagitems.length)],
  );
...
export const generateLevel = (objetivoId: number): string[][] => {
  ...
  const combinedArray = shuffledCharacters.map(
    (char, index) => `${char}-${shuffledItems[index]}`,
  );
